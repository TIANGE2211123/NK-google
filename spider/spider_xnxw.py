...
# 创建输出文件夹
os.makedirs('./tjuxnxw', exist_ok=True)
...
# 定义URL列表
url_list = [f'https://news.tju.edu.cn/xnxw1/qb.htm'] + [f'https://news.tju.edu.cn/xnxw1/qb/{1156 - i + 1}.htm' for i in range(1, 1156)]
    ...
    async with coroutine:
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True, timeout=10, headers=headers) as client:
                ...
                # 保存页面内容到本地
                async with aiofiles.open(f'./tjuxnxw/{title}.html', mode='w', encoding='utf-8') as file:
